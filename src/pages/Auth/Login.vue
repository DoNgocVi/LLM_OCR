  ...
  <div class='mx-a min-w-[350px]'>
    ...
    <div class='mt-4 -ml-20'>
      <n-form label-placement="left" label-width="auto" require-mark-placement="right-hanging">
        <div class="mb-4">
          <n-form-item :label="$t('common.email')" :validation-status="validationStatus('email')"
            :feedback="errors.email">
            <n-input v-model:value="form.email" placeholder="Enter your email" type='text' />
          </n-form-item>
        </div>
        <div class="mb-4">
          <n-form-item :label="$t('common.password')" :validation-status="validationStatus('password')"
...
const validationStatus = (field: 'email' | 'password') => {
  if (v$.value[field].$dirty && v$.value[field].$error) {
    errors[field] = v$.value[field].$invalid
